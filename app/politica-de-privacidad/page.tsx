...
function PrivacyContent() {
  ...
  const content = lang === 'es' ? data.privacy_es : data.privacy_en;

  const t = {
    back: lang === 'es' ? 'Volver' : 'Back',
    label: lang === 'es' ? 'Legal' : 'Legal',
    title: lang === 'es' ? 'Política de Privacidad' : 'Privacy Policy',
    responsible: lang === 'es' ? 'Responsable del Tratamiento' : 'Data Controller',
    ...
    legalLink: lang === 'es' ? 'Aviso Legal →' : 'Legal Notice →',
          ...
          >
            ...
            {!loading && (
              <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-3">
                <h2 className="text-[10px] uppercase tracking-widest font-black text-white/30">{t.responsible}</h2>
            ...
            <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row gap-4 text-xs uppercase tracking-widest font-bold text-white/30">
              <Link href={`/aviso-legal?lang=${lang}`} className="hover:text-white transition-colors">{t.legalLink}</Link>
